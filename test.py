...
def validation(model, criterion, evaluation_loader, converter, opt):
    ...
    return valid_loss_avg.val(), accuracy, norm_ED, preds_str, confidence_score_list, labels, infer_time, length_of_data, batch_results
...
def test(opt):
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            for batch_idx, (preds, gts) in enumerate(batch_results):
                log.write(f"Batch {batch_idx + 1}:\n")
                for pred, gt in zip(preds, gts):
                    log.write(f"  Prediction: {pred}, Ground Truth: {gt}\n")
